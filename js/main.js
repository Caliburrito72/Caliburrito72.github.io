// js/main.js (AR reliability + non-sticky glows + planned layout enforcement)
(() => {
  ...
  // Constants and world
  const TILE = 16;
  const SPEED = 3.5 * TILE;
  const DIAG = 1 / Math.sqrt(2);
  const WORLD_W = MAP.width * TILE, WORLD_H = MAP.height * TILE;

  // Zoom and view
  let zoomScale = 1.25;

  // Stage letterbox for 16:9 and crisp DPR setup
  ...
  function fitCanvas() {
    ...
    ctx.imageSmoothingEnabled = false;
    // Keep glow buffer in sync with canvas internal size
  ...
  function onTouch(e){ const t=e.touches[0], r=joy.getBoundingClientRect(), cx=r.left+r.width/2, cy=r.top+r.height/2;
    const dx=t.clientX-cx, dy=t.clientY-cy, R=r.width/2-8, m=Math.hypot(dx,dy);
    touchState.x = Math.max(-1, Math.min(1, (m>R?(dx/m)*R:dx)/R));
    touchState.y = Math.max(-1, Math.min(1, (m>R?(dy/m)*R:dy)/R));
    touchState.active = true; e.preventDefault(); }
  btnE.addEventListener('touchstart', (e)=>{ tryInteract(); e.preventDefault(); }, { passive:false });

  // Glow buffer: offscreen, cleared at start of every render; halos drawn in screen space with zoom
  ...
  function init(){ UI.setHint('WASD/Arrows · E interact · Esc close · Touch joystick + E on mobile'); requestAnimationFrame(loop); }
  function loop(ts){
  ...
  function updateCamera(dt){
    const dpr = Math.max(1, window.devicePixelRatio || 1);
    const baseW = 640, baseH = 360;
    const viewW = (baseW * dpr) / zoomScale, viewH = (baseH * dpr) / zoomScale;
  ...
  function render(dt){
    ...
    // Always clear glow at start of frame to avoid sticky halos
    glowCtx.clearRect(0,0,glow.width,glow.height);

    // World: apply zoom then camera transform
    ctx.save();
    ...
    // Composite glow (additive)
    compositeGlow();
    drawMinimap();
  }

  // World visuals (unchanged logic for hills/ground/water/props; glow now uses screen coords with zoom)
  function drawParallax(){ const t=performance.now()*0.02; drawHills('#0b1634','#15214a',36,1.4,t*0.05); drawHills('#12204a','#1a2c62',72,1.8,t*0.07); drawHills('#1a2b60','#213874',108,2.2,t*0.09); drawFog(); }
  function drawHills(c1,c2,yOff,amp,t){ ctx.beginPath(); const baseY=MAP.height*TILE-(90+yOff); ctx.moveTo(0, MAP.height*TILE); for(let x=0;x<=MAP.width*TILE;x+=8){ const y=baseY + Math.sin((x+t)*0.02)*(amp*12); ctx.lineTo(x,y);} ctx.lineTo(MAP.width*TILE, MAP.height*TILE); ctx.closePath(); const g=ctx.createLinearGradient(0,baseY,0,MAP.height*TILE); g.addColorStop(0,c1); g.addColorStop(1,c2); ctx.fillStyle=g; ctx.fill(); }
  function drawFog(){ const fogTop=(MAP.height*TILE)*0.5; const grd=ctx.createLinearGradient(0,fogTop,0,MAP.height*TILE); grd.addColorStop(0,'rgba(173,216,230,0.04)'); grd.addColorStop(1,'rgba(173,216,230,0.16)'); ctx.fillStyle=grd; ctx.fillRect(0,0,MAP.width*TILE,MAP.height*TILE); }
    ...
    for(let ty=0; ty<MAP.height; ty++){
      for(let tx=0; tx<MAP.width; tx++){
        ...
        if (!solid) {
          ...
          ctx.fillStyle=`rgb(${20},${g},${b})`; ctx.fillRect(x,y,TILE,TILE);
  ...
  function drawWater(){
    ...
    for (const r of MAP.water.rects){
      ...
      const g=ctx.createLinearGradient(0,y0,0,y1); g.addColorStop(0,'#0f4364'); g.addColorStop(1,'#0b2a46'); ctx.fillStyle=g; ctx.fillRect(x0,y0,w,h);
      ctx.globalAlpha=0.12; ctx.fillStyle='#cfe6f5'; for(let y=y0;y<y1;y+=4){ const ph=Math.sin((y*0.08)+t*2.5)*2; ctx.fillRect(x0, y+ph, w, 1); }
      ctx.globalAlpha=0.10; ctx.fillStyle='#9adfff'; for(let y=y0+2;y<y1;y+=6){ const ph=Math.sin((y*0.06)-t*3.0)*3; ctx.fillRect(x0+4+ph, y, w-8, 1); }
      ctx.globalAlpha=1.0; ctx.fillStyle='rgba(255,255,255,0.08)'; ctx.fillRect(x0, y0, w, 2); ctx.fillRect(x0, y1-2, w, 2);
    }
  }
  ...
  function drawProps(){
    ...
    for(const p of MAP.props){
      ...
      if (p.type==='lamp'){
        ...
        // draw glow in screen space using camera + zoom
        ...
        ctx.fillStyle='#3aa26b'; ctx.beginPath(); ctx.arc(x-2, y, 3, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.arc(x+2, y, 3, 0, Math.PI*2); ctx.fill();
      } else if (p.type==='banner'){
        const color=p.color||'#ff7ae6'; ctx.fillStyle=color; ctx.fillRect(x-1, y-10, 2, 10); ctx.fillRect(x-6, y-10, 12, 4);
      } else if (p.type==='bench'){
        ctx.fillStyle='#5a4b3a'; ctx.fillRect(x-8, y-3, 16, 3); ctx.fillStyle='#3a2f24'; ctx.fillRect(x-7, y, 2, 3); ctx.fillRect(x+5, y, 2, 3);
      }
    }
  }
  ...
  function drawObjects(){
    for (const obj of MAP.objects){
      ...
      // glow as screen-space circle aligned with zoomed position
  ...
  function compositeGlow(){
    ...
    // extra safety: clear after composite
    glowCtx.clearRect(0,0,glow.width,glow.height);
  }

  // Minimap
  function drawMinimap(){
    const MM_W=96, MM_H=64, scaleX=MM_W/WORLD_W, scaleY=MM_H/WORLD_H;
    mmctx.clearRect(0,0,MM_W,MM_H); mmctx.fillStyle='rgba(8,12,22,0.85)'; mmctx.fillRect(0,0,MM_W,MM_H);
    mmctx.fillStyle='rgba(27,36,51,0.95)';
    for (let y=0;y<MAP.height;y++){ for (let x=0;x<MAP.width;x++){ if (MAP.solids[y][x]===1){
      mmctx.fillRect(Math.floor(x*TILE*scaleX), Math.floor(y*TILE*scaleY), Math.ceil(TILE*scaleX), Math.ceil(TILE*scaleY));
    } } }
    if (MAP.water && MAP.water.rects){ mmctx.fillStyle='rgba(110,231,255,0.5)'; for (const r of MAP.water.rects){
      mmctx.fillRect(Math.floor(r.x0*TILE*scaleX), Math.floor(r.y0*TILE*scaleY), Math.ceil((r.x1-r.x0)*TILE*scaleX), Math.ceil((r.y1-r.y0)*TILE*scaleY));
    } }
    mmctx.fillStyle='#6ee7ff'; for (const obj of MAP.objects){ const ox=obj.x*TILE+TILE/2, oy=obj.y*TILE+TILE/2; mmctx.fillRect(Math.floor(ox*scaleX)-1, Math.floor(oy*scaleY)-1, 2, 2); }
    mmctx.fillStyle='#ff7ae6'; mmctx.fillRect(Math.floor(player.x*scaleX)-2, Math.floor(player.y*scaleY)-2, 4, 4);
    mmctx.strokeStyle='rgba(207,230,245,0.85)'; mmctx.lineWidth=1;
    mmctx.strokeRect(Math.floor(camera.x*scaleX), Math.floor(camera.y*scaleY), Math.floor(camera.w*scaleX), Math.floor(camera.h*scaleY));
  }

  // Expose quick tuning
  window.__JRPG = { player, camera, MAP, UI, Entities, setZoom(z){ zoomScale = Math.max(1, Math.min(2, z)); }, fit: fitCanvas };
  init();
})();
