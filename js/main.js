// js/map.js (water band, denser props, more ambient NPCs)
const MAP = (() => {
  const TILE = 16;
  ...
  // Shallow water band (walkable false = 1 in solids would block; here we keep it walkable for now)
  // Mark region via a helper for main.js to render water visuals over ground
  const water = {
    // simple rectangle region: x0,y0,x1,y1 in tiles
    rects: [
      { x0: 20, y0: 26, x1: 36, y1: 30 } // a pond-like strip for reflections
    ]
  };

  // Decorative props (denser clusters)
  const props = [
    // Lamps framing the plaza
    { type: 'lamp', x: 9,  y: 19, emissive: true },
    { type: 'lamp', x: 13, y: 19, emissive: true },

    // Signs and banners guiding flows
